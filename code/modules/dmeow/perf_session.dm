...
/datum/dmeow_perf_session
	...
	/// only the headless runner sets this. a panel-driven run shares this datum on
	/// a live server, where deleting the crew and refusing to measure a noisy
	/// station would both be rude.
	var/quiet_station = FALSE
	/// how many clientless living mobs the runner deleted before starting.
	var/mobs_deleted = 0
	...
	/// active turfs outside the room at the start of the current pass. refreshed
	/// on a fresh pass only, same as SSair.queue_at_pass_start.
...
/// counted before the timed region in air/fire(), not inside it - the published
/// turf_delta has to stay the cost of process_active_turfs and nothing else.
///
/// gated on the interleave so the loop never runs during warmup, which is the
/// only stretch where the auto-tier observer could promote it. otherwise it would
/// go native in one arm and interpreted in the other, inside SSair's budget.
...
/datum/dmeow_perf_session/proc/freeze()
	timer_id = null
	// before anything else in here: the abort has to happen while phase is still
	// "warmup", or record_turf_row() starts writing rows for a round we refuse to
	// measure.
 ...
 * names the areas and tears the round down.
 *
 * refusing rather than warming up longer on purpose: a station that hasn't
 * settled in 180s has something feeding it, and the area list is the answer to
 * *what* - a longer warmup would just hide it.
 */
...
/datum/dmeow_perf_session/proc/record_turf_row(entry_part, pass_fresh, turf_delta)
	...
	UNTYPED_LIST_ADD(turf_rows, list(
		...
		// so on its own it can't say whether the arms diverged on the fixture or
		// on the station.
