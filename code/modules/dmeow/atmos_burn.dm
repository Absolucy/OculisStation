...
/// is this turf one of ours? a bounding box and not `cell in interior`, which is
/// a linear scan of ~1600 entries per lookup and this gets asked once per active
/// turf per SSair fire.
///
/// the box rather than "same z", because the reserved z holds other
/// reservations too and they'd read as ours.
...
/// how many active turfs aren't the room's. the whole point of the burn round is
/// that the number is 0 - anything else is the station being measured alongside
/// the fixture, and `queue_at_start` can't tell the two apart because it's the
...
/// the same set broken down by area type, for the log line that names what's
/// keeping the station awake. only called once, at the freeze gate.
