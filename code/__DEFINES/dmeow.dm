...
/// can reserve it. the room takes `size + 2` for its walls, `_reserve_area()`
/// wants a one-turf cordon around that, and a reservation level only offers
/// `world.maxx - 2 * SHUTTLE_TRANSIT_BORDER`. so `size + 2 + 2 + 32`, rounded
/// up to leave a little slack for the free-list scan.
