...
/datum/antagonist/vampire/proc/break_masquerade(mob/admin)
	...
	owner.current.playsound_local(null, 'modular_oculis/modules/vampires/sound/masquerade_violation.ogg', 100, FALSE, pressure_affected = FALSE)
...
/datum/antagonist/vampire/proc/give_masquerade_infraction(ignore_cooldown = FALSE)
	...
	owner.current.playsound_local(null, 'modular_oculis/modules/vampires/sound/lunge_warn.ogg', 100, FALSE, pressure_affected = FALSE)
...
/datum/antagonist/vampire/proc/adjust_humanity(count, silent = FALSE)
	...
	if(count > 0)
		...
		if(humanity < temp_humanity && !silent)
			owner.current.playsound_local(null, 'modular_oculis/modules/vampires/sound/humanity_gain.ogg', 50, TRUE, pressure_affected = FALSE)
			if(power_given)
		...
		if(humanity > temp_humanity && !silent)
			owner.current.playsound_local(null, 'modular_oculis/modules/vampires/sound/humanity_loss.ogg', 50, TRUE, pressure_affected = FALSE)
			if(power_removed)
