...
/datum/antagonist/vampire/proc/handle_blood()
	...
	if(current_vitae < VAMPIRE_LOW_BLOOD_WARNING && !low_blood_alerted)
		owner.current.playsound_local(null, 'modular_oculis/modules/vampires/sound/bloodneed.ogg', 100, FALSE, pressure_affected = FALSE)
...
/datum/antagonist/vampire/proc/final_death(skip_destruction = FALSE)
	...
	if(!skip_destruction)
		if(iscarbon(body))
			...
			playsound(owner.current, 'modular_oculis/modules/vampires/sound/burning_death.ogg', 100, TRUE)
		else
